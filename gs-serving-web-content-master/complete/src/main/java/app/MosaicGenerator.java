...
import app.palettegenerator.ColorPaletteGenerator;
import app.palettegenerator.ImageBasedColorPaletteGenerator;
 ...
 * - {@link MosaicGenerator#generateCompositeMosaic(BufferedImage, Integer, Integer, ColorPaletteGenerator)} - Returns an image containing all permutations of widths and color ranges.
 * - {@link MosaicGenerator#run(String, String, Integer, Integer)} - Generates and saves a composite image. Use this if you don't intend to further manipulate the image.
...
public class MosaicGenerator {
    ...
    public void run(String inputUrl, String outputDirectory, Integer colorCounts, Integer widths) throws MalformedURLException, IOException {
        ...
        imageByte = decoder.decodeBuffer(inputUrl);
     ...
     */
    public BufferedImage generateCompositeMosaic(BufferedImage sourceImage,Integer colorCounts, Integer widths, ColorPaletteGenerator paletteGenerator) {
        int labelOffsetY = 10;
        int cellWidth = sourceImage.getWidth();
        int cellHeight = sourceImage.getHeight();
        int containerWidth = sourceImage.getWidth();
        int containerHeight = sourceImage.getHeight();
        ...
        //for (Integer colorCount : colorCounts) {
            destX1 = 0;
            List<Color> colors = paletteGenerator.generateColorPalette(colorCounts);
            //for (Integer width : widths) {
                BufferedImage tileImage = generateMosaic(sourceImage, widths, colors);
                ...
                //graphics.drawString(srcX2 + "x" + srcY2 + "-" + colorCounts + "color", destX1, destY1 + labelOffsetY);
                destX1 = destX2;
                destX2 += cellWidth;
           // }
            destY1 = destY2;
